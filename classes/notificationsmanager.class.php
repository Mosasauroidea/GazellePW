...
class NotificationsManager {
    ...
    public function load_news() {
        ...
        if ($MyNews < $CurrentNews) {
            $this->create_notification(self::NEWS, $CurrentNews, "公告", "index.php", self::IMPORTANT);
        }
    }
    ...
    public function load_staff_pms() {
        ...
        if ($NewStaffPMs > 0) {
            $Title = Lang::get('pub', 'you_have_space') . ($NewStaffPMs == 1 ? Lang::get('pub', 'a') : $NewStaffPMs) . Lang::get('pub', 'space_new_staff_pm') . ($NewStaffPMs > 1 ? Lang::get('pub', 's') : '');
    ...
    public function load_inbox() {
        ...
        if ($NewMessages > 0) {
            $Title = Lang::get('pub', 'you_have_space') . ($NewMessages == 1 ? Lang::get('pub', 'a') : $NewMessages) . Lang::get('pub', 'space_new_message') . ($NewMessages > 1 ? Lang::get('pub', 's') : '');
    ...
    public function load_torrent_notifications() {
        ...
        if (!empty($NewNotifications)) {
            $Title = Lang::get('pub', 'you_have_space') . ($NewNotifications == 1 ? Lang::get('pub', 'a') : $NewNotifications) . Lang::get('pub', 'space_new_torrent_notification') . ($NewNotifications > 1 ? Lang::get('pub', 's') : '');
    ...
    public function load_collage_subscriptions() {
        if (check_perms('site_collages_subscribe')) {
            ...
            if ($NewCollages > 0) {
                $Title = Lang::get('pub', 'you_have_space') . ($NewCollages == 1 ? Lang::get('pub', 'a') : $NewCollages) . Lang::get('pub', 'space_new_collage_update') . ($NewCollages > 1 ? Lang::get('pub', 's') : '');
    ...
    public function load_quote_notifications() {
        if (isset(G::$LoggedUser['NotifyOnQuote']) && G::$LoggedUser['NotifyOnQuote']) {
            ...
            if ($QuoteNotificationsCount > 0) {
                $Title = Lang::get('pub', 'new_quote') . ($QuoteNotificationsCount > 1 ? Lang::get('pub', 's') : '');
    ...
    public function load_subscriptions() {
        ...
        if ($SubscriptionsCount > 0) {
            $Title = Lang::get('pub', 'new_subscription') . ($SubscriptionsCount > 1 ? Lang::get('pub', 's') : '');
            $this->create_notification(self::SUBSCRIPTIONS, 0, $Title, 'userhistory.php?action=subscriptions', self::INFO);
        }
    }

    public static function clear_news($News = false) {
        $QueryID = G::$DB->get_query_id();
        if (!$News) {
    ...
    public static function clear_blog($Blog = false) {
        $QueryID = G::$DB->get_query_id();
        if (!$Blog) {
