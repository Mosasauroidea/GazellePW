...
class Forums {
    ...
    public static function get_thread_info($ThreadID, $Return = true, $SelectiveCache = false) {
        if ((!$ThreadInfo = G::$Cache->get_value('thread_' . $ThreadID . '_info')) || !isset($ThreadInfo['Ranking'])) {
            ...
            G::$DB->set_query_id($QueryID);
            if (!$SelectiveCache || !$ThreadInfo['IsLocked'] || $ThreadInfo['IsNotice'] || $ThreadInfo['IsSticky']) {
    ...
    public static function check_forumperm($ForumID, $Perm = 'Read') {
        ...
        if (!empty($UserInfo['ExtraClasses'])) {
            foreach ($Forums[$ForumID]['Second'] as $SecondID) {
                if (isset($UserInfo['ExtraClasses'][$SecondID]) && $UserInfo['ExtraClasses'][$SecondID]) {
                    return true;
                }
            }
        }
        ...
        if (!$Threads) {
            G::$DB->query("SELECT
				...
				FROM forums_topics AS ft
                    ...
                    WHERE f.MinClassRead = $Class
				ORDER BY ft.LastPostID desc LIMIT 5");
            $Threads = G::$DB->to_array(MYSQLI_ASSOC);

            G::$Cache->cache_value("LatestThread", $Threads);
        }
        return $Threads;
    }
    ...
    public static function get_forums() {
        if (!$Forums = G::$Cache->get_value('forums_list')) {
            ...
            foreach ($Forums as $ForumID => &$Forum) {
                ...
                $Forum['Second'] = unserialize(base64_decode($Forum['Second']));
            }
     ...
     * @return bool
     */
    public static function is_unread($Locked, $Notice, $Sticky, $LastPostID, $LastRead, $LastTopicID, $LastTime) {
        return (!$Locked || $Notice || $Sticky) && $LastPostID != 0 && ((empty($LastRead[$LastTopicID]) || $LastRead[$LastTopicID]['PostID'] < $LastPostID) && strtotime($LastTime) > G::$LoggedUser['CatchupTime']);
    }
