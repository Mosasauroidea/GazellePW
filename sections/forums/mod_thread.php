...
$Hiddenreplies = isset($_POST['hiddenreplies']) ? 1 : 0;
$Locked = isset($_POST['locked']) ? 1 : 0;
$Notice = isset($_POST['notice']) ? 1 : 0;
$Ranking = (int)$_POST['ranking'];
$AutoLocked = 0;
if (isset($_POST['autolocked']) && ($_POST['autolocked'] == 1 || $_POST['autolocked'] == 2)) {
    $AutoLocked = $_POST['autolocked'];
}
/*
if (!$Sticky && $Ranking > 0) {
    $Ranking = 0;
...
$DB->query("
	...
	WHERE t.ID = '$TopicID'
	GROUP BY p.TopicID");
list($OldForumID, $OldForumName, $MinClassWrite, $Posts, $ThreadAuthorID, $OldTitle, $OldLocked, $OldNotice, $OldSticky, $OldRanking) = $DB->next_record(MYSQLI_BOTH, false);
...
if (isset($_POST['delete'])) {
    ...
    $DB->query("
		...
		LIMIT 1");
    list($NewLastTopic, $NewLastPostID, $NewLastTitle, $NewLastAuthorID, $NewLastAuthorName, $NewLastAddedTime, $NumPosts, $NewLocked, $NewNotice, $NewSticky) = $DB->next_record(MYSQLI_NUM, false);
    ...
    if ($ForumID != $OldForumID) { // If we're moving a thread, change the forum stats
        ...
        $DB->query("
			...
			ORDER BY t.LastPostID DESC
			LIMIT 1");
        list($NewLastTopic, $NewLastPostID, $NewLastTitle, $NewLastAuthorID, $NewLastAuthorName, $NewLastAddedTime, $NumPosts, $NewLocked, $NewNotice, $NewSticky, $NewRanking) = $DB->next_record(MYSQLI_NUM, false);
