...
$DB->query("
		...
		FROM users_info AS ui
			...
			LEFT JOIN users_levels AS ul ON ul.UserID = um.ID AND (ul.PermissionID = '" . CELEB . "' or ul.PermissionID = '57')
...
while (list($Username, $Email) = $DB->next_record()) {
	$Tpl = new \TEMPLATE;
	$Tpl->open(SERVER_ROOT . '/templates/inactive_notice.tpl');
	$Tpl->set('SiteURL', SITE_URL);
	$Tpl->set('UserName', $Username);
	Misc::send_email($Email, '你的 ' . SITE_NAME . ' 账号即将被封禁 | Your ' . SITE_NAME . ' account is about to be disabled', $Tpl->get(), 'noreply');
}
