...
preg_match('/id=(\d+)/', $P['fatherLink'], $matches);
if ($matches) {
    $FatherID = $matches[1];
} else {
    $FatherID = 0;
...
$DB->query("
	INSERT INTO wiki_articles
		(Revision, Title, Body, MinClassRead, MinClassEdit, Date, Author, Lang, Father)
	VALUES
		('1', '$P[title]', '$P[body]', '$Read', '$Edit', '" . sqltime() . "', '$LoggedUser[ID]', '$P[language]', '$FatherID')");
