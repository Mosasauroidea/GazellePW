...
if (isset($_GET['cache'])) {
    if ($_GET['cache'] === 'users') {
        ...
        echo "<div class='save_message'>{$Count} " . t('server.tools.users_caches_cleared') . "</div>";
...
<table class="layout" cellpadding="2" cellspacing="1" border="0" align="center">
    <tr>
        <td style="width:15%; text-align:right;"><?= t('server.tools.key') ?>:</td>
        <td>
    ...
    <tr data-tooltip="<?= $MultiKeyTooltip ?>">
        <td style="text-align:right;"><?= t('server.tools.multi_key') ?>:</td>
        <td>
            <form class="manage_form" name="cache" method="get" action="">
                <input type="hidden" name="action" value="clear_cache" />
                <select class="Input" name="type">
    ...
    <tr>
        <td rowspan="3" style="text-align:right;"><?= t('server.tools.clear_common_cashes') ?>:</td>
        <td><a href="tools.php?action=clear_cache&cache=users"><?= t('server.tools.users') ?></a> (<?= t('server.tools.clears_out') ?> user_stats_*, user_info_*, and user_info_heavy_*)</td>
    </tr>
    <tr>
        <td><a href="tools.php?action=clear_cache&cache=torrent_groups"><?= t('server.tools.torrent_groups') ?></a> (<?= t('server.tools.clears_out') ?> torrent_group_* and groups_artists_*)</td>
    </tr>
    <tr>
        <td><a href="tools.php?action=clear_cache&cache=torrent_checked"><?= t('server.tools.torrent_checked') ?></a> (<?= t('server.tools.clears_out') ?> torrent_checked_*)</td>
    </tr>
</table>
<?
