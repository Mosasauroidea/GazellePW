    ...
    <div class="TableContainer pad box" id="torrent_sale_management">
        <table id="torrent_sale_management_table">
            <tr>
                ...
                <td>
                    <select class="Input">
                        <option class="Select-option"><?= Lang::get('tools.all_torrents_include_new') ?></option>
                        <option class="Select-option"><?= Lang::get('tools.all_current_torrents') ?></option>
                        <option class="Select-option"><?= Lang::get('tools.all_new_torrents') ?></option>
                        <!-- <option class="Select-option"><?= Lang::get('tools.internal_torrents') ?></option> -->
                        <option class="Select-option"><?= Lang::get('tools.certain_torrent_groups') ?></option>
                        <option class="Select-option"><?= Lang::get('tools.certain_torrents') ?></option>
                        <!-- <option class="Select-option"><?= Lang::get('tools.certain_size_torrents') ?></option> -->
                    </select>
                </td>
            </tr>
            <tr class="hidden">
            ...
            <tr>
                ...
                <td>
                    <select class="Input">
                        <option class="Select-option"><?= Lang::get('global.type') ?></option>
                        <option class="Select-option"><?= Lang::get('global.feature_film') ?></option>
                        <option class="Select-option"><?= Lang::get('global.short_film') ?></option>
                        <option class="Select-option"><?= Lang::get('global.miniseries') ?></option>
                        <option class="Select-option"><?= Lang::get('global.stand_up_comedy') ?></option>
                        <option class="Select-option"><?= Lang::get('global.live_performance') ?></option>
                        <option class="Select-option"><?= Lang::get('global.movie_collection') ?></option>
                    </select>
                    <select class="Input">
                        <option class="Select-option"><?= Lang::get('tools.resolution') ?></option>
                        <option class="Select-option"><?= Lang::get('tools.standard_definition') ?></option>
                        <option class="Select-option">720p</option>
                        <option class="Select-option">1080p</option>
                        <option class="Select-option">2160p</option>
                    </select>
                    <select class="Input">
                        <option class="Select-option"><?= Lang::get('tools.processing') ?></option>
                    ...
                    <select class="Input">
                        <option class="Select-option"><?= Lang::get('tools.source') ?></option>
            ...
            <tr>
                ...
                <td>
                    <select class="Input">
                        <option class="Select-option"><?= Lang::get('tools.original_options') ?></option>
                        <option class="Select-option"><?= Lang::get('tools.both') ?></option>
                        <option class="Select-option"><?= Lang::get('upload.self_rip') ?></option>
                        <option class="Select-option"><?= Lang::get('upload.self_purchase') ?></option>
                    </select>
                    <select class="Input">
                        <option class="Select-option"><?= Lang::get('tools.internal_torrent') ?></option>
                        <option class="Select-option"><?= Lang::get('tools.yes') ?></option>
                        <option class="Select-option"><?= Lang::get('tools.no') ?></option>
                    </select>
                    <select class="Input">
                        <option class="Select-option"><?= Lang::get('tools.feature_torrent') ?></option>
                        <option class="Select-option"><?= Lang::get('tools.both') ?></option>
                        <option class="Select-option"><?= Lang::get('tools.chinese_dubbed') ?></option>
                        <option class="Select-option"><?= Lang::get('tools.special_effects_subtitles') ?></option>
                        <option class="Select-option"><?= Lang::get('tools.no') ?></option>
                    </select>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    <select class="Input">
                        <option class="Select-option"><?= Lang::get('tools.free_leech') ?></option>
                        <option class="Select-option"><?= Lang::get('tools.25_percent_off') ?></option>
                        <option class="Select-option"><?= Lang::get('tools.50_percent_off') ?></option>
                        <option class="Select-option"><?= Lang::get('tools.75_percent_off') ?></option>
                        <option class="Select-option"><?= Lang::get('tools.neutral_leech') ?></option>
                    </select>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                <td colspan="2" class="center">
                    <button class="Button"> <?= Lang::get('tools.add_rule') ?> </button>
                </td>
            </tr>
        </table>
    </div>
    ...
    <div id="current_sale_rules">
        <table class="TableMultipleFreeleech Table">
            <tr class="Tablep-rowHeader">
                <td class="Table-cell"><?= Lang::get('tools.sales_promotion_range') ?></td>
                <td class="Table-cell"><?= Lang::get('tools.sales_promotion_plan') ?></td>
                <td class="Table-cell"><?= Lang::get('tools.sales_promotion_period') ?></td>
                <td class="Table-cell"><?= Lang::get('tools.added_on') ?></td>
                <td class="Table-cell"><?= Lang::get('tools.operator') ?></td>
                <td class="Table-cell"><?= Lang::get('tools.operation') ?></td>
            </tr>
            <tr class="Table-row">
                <td class="Table-cell"><a><?= Lang::get('tools.click_to_see') ?></a></td><!-- 这样设计是因为考虑到自定义一批种子或种子组，需要另行显示；如果是全局的批量，就直接显示文字就可以。 -->
    ...
    <div id="sale_rules_history">
        <table class="TableMultipleFreeleech Table">
            <tr class="Table-rowHeader">
                ...
                <td class="Table-cell"><?= Lang::get('tools.sales_promotion_period') ?></td>
                <td class="Table-cell"><?= Lang::get('tools.added_on') ?></td>
                <td class="Table-cell"><?= Lang::get('tools.deleted_on') ?></td>
                <td class="Table-cell"><?= Lang::get('tools.operator') ?></td>
            </tr>
            <tr class="Table-row">
                <td class="Table-cell"><a><?= Lang::get('tools.click_to_see') ?></a></td><!-- 这样设计是因为考虑到自定义一批种子或种子组，需要另行显示；如果是全局的批量，就直接显示文字就可以。 -->
    ...
    <div class="BoxBody">
        <form class="send_form center" action="" method="post">
            ...
            <select class="Input" name="freeleechtype">
                <option class="Select-option" value="1" <?= $_POST['freeleechtype'] == '1' ? 'selected' : '' ?>><?= Lang::get('tools.fl') ?></option>
                <option class="Select-option" value="2" <?= $_POST['freeleechtype'] == '2' ? 'selected' : '' ?>><?= Lang::get('tools.nl') ?></option>
                <option class="Select-option" value="0" <?= $_POST['freeleechtype'] == '0' ? 'selected' : '' ?>><?= Lang::get('tools.normal') ?></option>
            </select>&nbsp;<?= Lang::get('tools.for_reason') ?>&nbsp;<select class="Input" name="freeleechreason">
