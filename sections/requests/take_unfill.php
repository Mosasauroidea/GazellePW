...
Misc::send_pm($FillerID, 0, Lang::get('inbox', 'request_filled_unfilled', $FillerLang), Lang::get('inbox', 'the_request_url', $FillerLang) . site_url() . "requests.php?action=view&amp;id=$RequestID]$FullName" . Lang::get('inbox', 'url_was_unfilled_by_url', $FillerLang) . site_url() . 'user.php?id=' . $LoggedUser['ID'] . ']' . $LoggedUser['Username'] . Lang::get('inbox', 'url_for_the_reason_quote', $FillerLang) . $_POST['reason'] . Lang::get('inbox', 'quote_if_disagree_unfill_please_url', $FillerLang) . site_url() . "reports.php?action=report&amp;type=request&amp;id=" . $RequestID . Lang::get('inbox', 'report_request_and_explain', $FillerLang));
