...
View::show_header('Recovery browse users', '', 'PageRecoveryHome');
?>
                    ...
                    <table>
                        ...
                        foreach ($List as $r) { ?>
                            <tr>
                                ...
                                <td><?= $r['UserID'] ?></td>
                                <td><?= $r['Email'] ?></td>
                                <td data-tooltip="<?= $r['Uploaded'] ?>"><?= Format::get_size($r['Uploaded']) ?></td>
                                <td data-tooltip="<?= $r['Downloaded'] ?>"><?= Format::get_size($r['Downloaded']) ?></td>
                                <td><?= $r['Enabled'] ?></td>
                                <td><?= $r['nr_torrents'] ?></td>
        ...
        <div class="pad">
            <form method="post" action="/recovery.php?action=browse">
                ...
                <table>
                    <tr>
                        <th>Username</th>
                        <td><input class="Input" type="text" name="username" width="20" /></td>
                    </tr>
                    <tr>
                        <th>Email</th>
                        <td><input class="Input" type="text" name="email" width="20" /></td>
                    </tr>
                    <tr>
                        <th>Announce</th>
                        <td><input class="Input" type="text" name="announce" width="20" /></td>
                    </tr>
                    <tr>
                        <td></td>
