...
<div class="LayoutBody">
    ...
    if ($Details == 'all' || $Details == 'day') {
        ...
        generate_torrent_table(Lang::get('top10.in_the_past_day', ['Values' => [Lang::get('top10.movies')]]), 'day', $Data, $Limit);
    }
    if ($Details == 'all' || $Details == 'week') {
        $Data = $Top10Movies->getData("active_week", ['Limit' => $Limit]);
        generate_torrent_table(Lang::get('top10.in_the_past_week', ['Values' =>  [Lang::get('top10.movies')]]), 'week', $Data, $Limit);
    }
    if ($Details == 'all' || $Details == 'month') {
        $Data = $Top10Movies->getData("active_month", ['Limit' => $Limit]);
        generate_torrent_table(Lang::get('top10.in_the_past_month', ['Values' => [Lang::get('top10.movies')]]), 'month', $Data, $Limit);
    }
    if ($Details == 'all' || $Details == 'year') {
        $Data = $Top10Movies->getData("active_year", ['Limit' => $Limit]);
        generate_torrent_table(Lang::get('top10.in_the_past_year', ['Values' =>  [Lang::get('top10.movies')]]), 'year', $Data, $Limit);
    }
    if ($Details == 'all' || $Details == 'overall') {
        $Data = $Top10Movies->getData("active_all", ['Limit' => $Limit]);
        generate_torrent_table(Lang::get('top10.most_torrents', ['Values' => [Lang::get('top10.movies')]]), 'overall', $Data, $Limit);
    }

    if (($Details == 'all' || $Details == 'snatched') && !$Filtered) {
        $Data = $Top10Movies->getData("snatched", ['Limit' => $Limit]);
        generate_torrent_table(Lang::get('top10.most_snatched', ['Values' => [Lang::get('top10.movies')]]), 'snatched', $Data, $Limit);
    }

    if (($Details == 'all' || $Details == 'data') && !$Filtered) {
        $Data = $Top10Movies->getData("data", ['Limit' => $Limit]);
        generate_torrent_table(Lang::get('top10.most_data', ['Values' => [Lang::get('top10.movies')]]), 'data', $Data, $Limit);
    }

    if (($Details == 'all' || $Details == 'seeded') && !$Filtered) {
        $Data = $Top10Movies->getData("seeded", ['Limit' => $Limit]);
        generate_torrent_table(Lang::get('top10.most_seed', ['Values' => [Lang::get('top10.movies')]]), 'seeded', $Data, $Limit);
    }
    ?>
</div>
<?
