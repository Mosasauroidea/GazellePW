...
;(function () {
  ...
  for (var i = 0, link, l = downloadlinkElms.length; i < l; i++) {
    if (
      downloadlinkElms[i].href.indexOf('action=download') != -1 &&
      downloadlinkElms[i].href.indexOf('usetoken=') == -1
    ) {
      link = document.createElement('a')
      link.textContent = 'JS'
      var txtNode = document.createTextNode(' | ')
      link.href =
        'ajax.php?action=torrent&id=' +
        downloadlinkElms[i].href.replace(/^.*?id=(\d+)&.*?$/, '$1')
      link.download =
