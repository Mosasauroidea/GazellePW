...
!(function ($) {
    ...
    tablesorter: new (function () {
      ...
      function buildParserCache(table) {
        ...
        if (rows[0]) {
          ...
          for (i = 0; i < l; i++) {
            ...
            h = h
              .add(c.$headers.filter('[colspan="1"]')) // ie8 fix
          ...
          if (!$(b[k]).hasClass(tc.cssInfoBlock)) {
            ...
            for (i = 0; i < totalRows; ++i) {
              ...
              if (c.hasClass(tc.cssChildRow)) {
                tc.cache[k].row[tc.cache[k].row.length - 1] =
                  tc.cache[k].row[tc.cache[k].row.length - 1].add(c)
        ...
        if (c.sortRestart) {
          ...
          c.$headers.each(function () {
            ...
            if (
              this !== indx &&
              (key ||
                !$(this).is('.' + ts.css.sortDesc + ',.' + ts.css.sortAsc))
            ) {
              this.count = -1
            }
          })
          ...
          cache.sort(function (a, b) {
            ...
            for (i = 0; i < l; i++) {
              ...
              if (num && c.strings[col]) {
                ...
                sort = c.numberSorter
                  ? c.numberSorter(x[col], y[col], dir, colMax[col], table)
          ...
          .bind(
            ...
            function (e, cell, resort, callback) {
              ...
              if ($tb.length && tbdy >= 0) {
                ...
                c.cache[tbdy].row[table.config.cache[tbdy].normalized[row][l]] =
                  $row
                c.cache[tbdy].normalized[row][icell] = c.parsers[icell].format(
      ...
      ts.bindEvents = function (table, $headers) {
        ...
        $headers
          ...
          .unbind(
            'mousedown mouseup sort keyup '.split(' ').join(c.namespace + ' ')
          )
          .bind(
            'mousedown mouseup sort keyup '.split(' ').join(c.namespace + ' '),
            function (e, external) {
              ...
              if (
                ((e.which || e.button) !== 1 && !/sort|keyup/.test(type)) ||
      ...
      ts.destroy = function (table, removeClasses, callback) {
        ...
        $r.find(c.selectorSort).unbind(
          'mousedown mouseup keypress '.split(' ').join(c.namespace + ' ')
        )
        ts.restoreHeaders(table)
      ...
      ts.getParserById = function (name) {
        ...
        for (i = 0; i < l; i++) {
          if (
            ts.parsers[i].id.toLowerCase() === name.toString().toLowerCase()
          ) {
            return ts.parsers[i]
          }
        }
            ...
            if (w) {
              if (init || !c.widgetInit[w.id]) {
                if (w.hasOwnProperty('options')) {
                  wo = table.config.widgetOptions = $.extend(
                    true,
                    {},
                    w.options,
                    wo
  ...
  ts.addParser({
    ...
    format: function (s, table) {
      return s
        ? ts.formatFloat(
            s !== '' ? new Date(s.replace(/-/g, '/')).getTime() || s : '',
            table
          )
        : s
    },
  ...
  ts.addParser({
    ...
    format: function (s, table) {
      return s
        ? ts.formatFloat(
            new Date(s.replace(/(\S)([AP]M)$/i, '$1 $2')).getTime() || s,
            table
          )
        : s
    },
      ...
      if (s) {
        var c = table.config,
          ...
          format =
            ...
            ts.getData(ci, c.headers[cellIndex], 'dateFormat') ||
            c.dateFormat
  ...
  ts.addParser({
    ...
    format: function (s, table) {
      return s
        ? ts.formatFloat(
            new Date(
              '2000/01/01 ' + s.replace(/(\S)([AP]M)$/i, '$1 $2')
            ).getTime() || s,
            table
          )
        : s
